...
void generateSeedsDeg(int n, int k, float f, const char* infFile, vector<int> &seeds) {
    ifstream in(infFile);
    int i, v1, rand_pos;
    float v2;
    int num = n * f;
...
int main(int argc, char ** argv)
{
    ...
    char * degFile = op.getPara("-d");
    if (degFile == NULL){
        degFile = (char*)"single_deg.txt";
    }

    char * infFile2 = op.getPara("-a");
    if (infFile2 == NULL){
        infFile2 = (char*)"already.seeds";
    }
    ...
    } else if (strcmp(method, "deg") == 0) {
        generateSeedsDeg(n, k, f, degFile,seeds);
